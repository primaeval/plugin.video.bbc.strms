...
def Service():
    ...
    servicing = True
    xbmc.log("SERVICE")
    xbmc.executebuiltin('XBMC.RunPlugin(plugin://plugin.video.bbc.strms/service)')
    time.sleep(2)
    servicing = False
...
if __name__ == '__main__':
    ...
    try:
        if ADDON.getSetting('service') == 'true':
            ...
            xbmc.log("[plugin.video.bbc.strms] service started...", xbmc.LOGERROR)
            ...
            while not monitor.abortRequested():
                if ADDON.getSetting('service.type') == '1':
                    ...
                    xbmc.log("[plugin.video.bbc.strms] Service waiting for interval %s" % waitTime, xbmc.LOGERROR)
                ...
                if timeLeft <= 0:
                    timeLeft = 1
                xbmc.log("[plugin.video.bbc.strms] Service waiting for %d seconds" % timeLeft, xbmc.LOGERROR)
                if timeLeft and monitor.waitForAbort(timeLeft):
                    break
                xbmc.log("[plugin.video.bbc.strms] Service now triggered...", xbmc.LOGERROR)
                Service()
                now = time.time()
